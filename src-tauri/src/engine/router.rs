use crate::engine::commands::{
    ...
    message, orient, provider, role, skill, skill_draft, snapshot, task, tool, usage, workspace,
};
...
pub async fn dispatch(state: &AppState, cmd: &str, payload: Value) -> Result<Value, AppError> {
    match cmd {
        ...
        "instance.cliModels" => instance::cli_models(state, payload).await,
        ...
        // ── usage ─────────────────────────────────────────────────────────
        "usage.overview" => usage::overview(state, payload).await,
