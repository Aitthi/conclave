...
`proxy_checkpoint_metric.rs` (mirror `proxy_metric.rs`: raw `sqlx::query` INSERT with `?1..?18`; `report` via `QueryBuilder::<Sqlite>` with `COUNT(*) AS samples`, `SUM(CASE WHEN count_failure = 0 THEN 1 ELSE 0 END) AS eligible`, `AVG(q) AS avg_q`, `AVG(projected_post_tokens) AS avg_projected_post_tokens`, `MAX(plateau_turns) AS max_plateau_turns`, `SUM(count_failure) AS count_failures`, `where_gte("created_at", cutoff)`, `fetch_one::<CheckpointReport, _>`). Use `super::cb_err` for the chain-builder error map, exactly as `proxy_metric::report` does. Add `pub mod proxy_checkpoint_metric;` to `repo/mod.rs`.
