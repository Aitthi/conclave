...
- `src-tauri/src/engine/commands/instance.rs`
  ...
  - Add focused async/unit tests for command construction, successful parsing, empty/malformed success, non-zero exit, and unsupported `cliKind`.
...
- `agy models` is network/auth dependent and can fail while the executable itself is installed. Keep availability and catalog failures distinct.
