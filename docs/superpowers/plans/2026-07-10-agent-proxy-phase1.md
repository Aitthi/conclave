...
- Modify: `src-tauri/src/engine/db.rs` (`if version < 20` gate), `src-tauri/src/engine/repo/agent_definition.rs` (`pub proxy_enabled: Option<bool>` beside `rtk_enabled` `:121`, plus the column in its SELECT list and update path — mirror `rtk_enabled` everywhere it appears), `src-tauri/src/engine/commands/agent.rs` (2-line plumbing ONLY: request field beside `rtk_enabled` `:86`, input construction beside `:314` — added by ruling on challenge 277b8235; outside the immutable task boundary, so it lands as its OWN scoped commit `git commit -- src-tauri/src/engine/commands/agent.rs`), `src-tauri/src/engine/commands/instance.rs` (env block 755-785), `src-tauri/src/engine/runtime/sandbox_config.rs` (network allowlists)
...
Sandbox: in `sandbox_config.rs`, wherever the Claude settings (`claude_sandbox_settings` `:60`) and Codex overrides (`codex_socket_overrides` `:41`) enumerate allowed network destinations, add the loopback HOST `127.0.0.1` when (and only when) the base-URL injection fired — thread the decision through the existing call sites in `instance.rs:629-733` rather than allowlisting unconditionally. (Host only, NO port: neither claude's `sandbox.network.allowedDomains` nor codex's `permissions.<profile>.network.domains` accepts port syntax — amendment A6, memory c604ed06.)
...
- **A4 (2026-07-11, lane-B review note 3233be4f, deferred follow-ups — candidates for Phase 2 or a cleanup lane, do NOT fix in Phase 1 lanes):** N2 proxy bind-retry eprintlns every 1s forever when the port is permanently held → cap or back off; N3 non-streaming (`stream:false`) responses yield no usage (SSE-only parser) — agent CLIs always stream, fine for v1; N4 a mid-request mode flip can record `mode='off'` outside the column comment's domain (no CHECK constraint, harmless); N5 `on_request_complete` re-observes with the completed request's messages and can briefly rewind stored hashes / create one phantom ledger entry post-eviction (prefix-match + cap 64 absorb it). N1 (per-request reqwest client) was fixed pre-merge in the lane.
- **A5 (2026-07-11, ruling on Dew's challenge 277b8235):** Task 11's boundary omitted `src-tauri/src/engine/commands/agent.rs`, which `AgentDefinitionInput`'s exhaustive construction (`agent.rs:294`, plumbing at `:86`/`:314`) forces into scope — plan defect (found by Dew; third instance of the boundary-omits-required-wiring class, see memory df65b613). Ruled: widen to that file, plumbing lines only, landed as a separate scoped commit since the stored task boundary is immutable. Task 11 Files list amended in place.
- **A6 (2026-07-11, lane-C review note e9cc3b86):** the plan's original `127.0.0.1:<port>` allowlist wording was never valid syntax — both claude `sandbox.network.allowedDomains` and codex `permissions.<profile>.network.domains` are HOST-only (verified by Dew against the settings schema + codex 0.144.1 binary, independently re-verified by Mellow; memory c604ed06). Task 11 text amended; implemented host-only. Two informational carry-overs for the live gate & Phase 2: the agent's zsh profile can override any injected env var incl. `ANTHROPIC_BASE_URL` (check when the Step-6 live gate misbehaves), and a codex agent opting in gets a no-op env var but a real (harmless) sandbox hole.
...
- **A7 (2026-07-11, from Dew's Step-6 live-gate finding F2, task note 15c8ae50):** proxy usage metrics are all NULL in live traffic — the client's `accept-encoding` is forwarded verbatim upstream, so the SSE tee in `src-tauri/src/engine/runtime/ctx_proxy.rs` receives compressed bytes its usage parser cannot parse, AND `ledger.last_input_tokens` never gets set, silently disarming the rewrite-mode high-water trigger (D4 hysteresis). This BLOCKS Phase-1 exit acceptance step 1 (real would-save numbers). Ruled: fix NOW in follow-up lane `agent-proxy-usage-encoding` (not deferred with N2–N5, because measurement is blocked). Approach: request identity encoding on the UPSTREAM request (strip/override `accept-encoding`); decompressing in the tee is the fallback only if identity is refused. F1 from the same gate (`conclave send` types without submitting) is BY DESIGN — `message.send`'s contract says the frontend appends the newline (`message.rs:31-34` doc comment); CLI callers use `tell`/inject, which auto-submits. No code change for F1.
- **A8 (2026-07-11, measurement finding + human "continue proxy" directive):** first live measurement window (116 requests) shows agents on fable-5 (1M window) self-compact at ~70% context — exactly `HIGH_WATER = 0.70` — so elision essentially never fires for them (115 passthrough; the one `reevaluate` came from an opus-4-8 smaller-window session: 3 elisions, 46,839 bytes would-save on a 656 KB request, ~7%). Phase-1 exit acceptance step 1 (≥25% reduction on a real session) is unmeasurable at a fixed 0.70, and a production threshold equal to the self-compact point is a no-op by construction. Ruled: make the threshold runtime-settable via `conclave proxy threshold <ratio>` (same atomic pattern and restart-reset semantics as `proxy mode`; default stays 0.70) — follow-up lane `proxy-threshold-config`. After it lands + app rebuild, set threshold to ~0.25 in log mode to collect would-save data from real lanes, then present the rewrite go/no-go. Options (b) accept tail-end-only and (c) measure only on small-window models are subsumed: (a)-with-runtime-tuning dominates both.
- **A9 (2026-07-11, go/no-go RULING — two-principal consensus Detoro + Aoki, in-loop authority granted by human):** Phase-1 dedup ctx-proxy is **NO-GO for rewrite** and the rewrite path is **SHELVED** for the Claude fleet. Measurement at threshold 0.25/log with an ample sample (1004 requests, 368 eliding across opus-4-8/sonnet-5/opus-4-7/fable-5, sqlite `proxy_request_metric`): would-save reduction = **3.4% aggregate** (14.87 MB / 437 MB request bytes), **best single request 9.5%**, **0/1004 reach 10%**; fable-5 (1M window) = 113 passthrough / 0 elisions (self-compacts below the 0.25 high-water). vs the A8/exit bar of ≥25% this fails ~7×. **Objective reframe (the core finding):** "input reduction" is the wrong primary objective under prompt caching. Correct objective = expected total cost = cache_creation + cache_read + uncached_input (+ latency), subject to outcome-equivalence + context-survival. This fleet's cache-read (193 M tok) dwarfs raw input (356 K tok), so eliding *cached* mid-history busts the cache-prefix suffix → cache_creation (~1.25×) on the tail vs the saved bytes (~0.1× cache_read). Rewrite amortizes only after n ≥ (1.15R−1.25S)/(0.1S) later *stable* rounds (Aoki's math; ~12 rounds for the 9.5% case) — not observed in real traffic, where agents self-compact and cuts change round-to-round. **Not a universal claim:** rewrite CAN be positive-EV in narrow regimes preserved as future research hypotheses — (1) caching off/low hit-rate, (2) trimming a long imported/resumed prompt BEFORE first cache creation, (3) changing only the uncached tail after the last cache breakpoint, (4) a stable compaction checkpoint + enough amortizing rounds, (5) avoiding context overflow / an imminent larger harness compaction where continuity value dominates. Current policy has no cache-boundary awareness, so none apply today. **Operational:** keep the proxy in **mode=log** (NOT off — `MODE_OFF` at ctx_proxy.rs:130-133 bypasses `rewrite_body` AND the SSE usage tee, killing all metrics) for continued observability; threshold restored to the 0.70 default (fewer analysis crossings; log still observes every request). **No rebuild required** — shelving is runtime-only and the default (log@0.70) is what every relaunch already lands on. A rebuild is needed ONLY to enforce a persistent OFF or to remove proxy routing/default-on itself; not doing that. Phase 2 (semantic/lossy compaction of *unique* history to chase 25%) is NOT opened; if ever pursued it must optimize the total-cost objective above with cache-boundary awareness, and it is a new spec with much harder equivalence + cache + observability requirements. Evidence: blackboard `measure:proxy-025-verdict` (raw measurement) + `measure:proxy-025-ruling` (this ruling). Credit: Aoki corrected Detoro's over-strong "rewrite can only lose" universal to the fleet-specific, amortization-bounded form recorded here.
