...
- codex: `codex exec --json --ephemeral --skip-git-repo-check -o <last.json> [-m <m>] -` with the prompt on stdin; result = parsed contents of `<last.json>` after `strip_code_fences`. NO `--output-schema`: R2 fired (Dew, 2026-09-04) — OpenAI Structured Outputs rejects the draft schema with HTTP 400 before the model runs because strict mode demands every property in `required`, while the draft contract has optional fields by design; the same prompt without the flag returned clean, catalogue-only JSON. The schema still travels inside the prompt text and the Rust validator remains the guard for both CLIs. `codex_launch(model: Option<&str>, out_path: &Path) -> String`.
