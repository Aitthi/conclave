...
| D8 | Model catalogue is lifted from `Builder.tsx:74-90` into a shared TS constant AND mirrored as a Rust constant; both lists are passed to the drafter and checked by the validator. Codex context window is not drafted (derived backend-side, `codex_models.rs`). | *Let the model invent model ids* rejected (D4). |
