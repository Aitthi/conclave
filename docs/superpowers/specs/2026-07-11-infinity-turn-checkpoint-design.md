...
The 100k acquisition ceiling is a caveat, not a reason to reverse the verdict: M2 did not sample the high-context band. Hybrid may proceed to shadow measurement only if its final GO decision includes real samples in the binding measurement band — **250–350k real-A tokens** (human directive 2026-07-12, R10; originally 400–500k); 100k samples may calibrate the instrument but cannot establish the product claim.
...
**H1 — shadow token/economics measurement, no forwarded-byte change.** Re-arm the existing sampler after app relaunch, generate candidate summaries off-path, and reuse M1's a/b/c instrument. Acquire at least **30 successful candidates across at least 10 conversations**, including at least **10 candidates whose real A is in the 250–350k measurement band** (R10; originally the 400–500k degradation band); 100k samples are calibration only. Report distributions by conversation, never only pooled averages. Advance to H2 only if: count/generation failure ≤5%; at least 90% of candidates land at or below the defined low-water L; at least 80% have `n_h ≤ 2` using measured `C_gen`; and no conversation has all candidates miss. These retain M2's two-turn cost standard while charging the new call honestly.
...
- **R10 (human directive, 2026-07-12) H1 measurement band lowered to 250–350k:** the binding H1 in-band acquisition requirement moves from real A ∈ [400k, 500k] to **real A ∈ [250k, 350k]**, and the default sampler ceiling moves 450k → **300k** (preserving the band = ceiling ± 50k geometry: the byte-estimate trigger admits at estimate > ceiling and real A distributes around it). Corollary (challenge 42cc2a0f, credit Dabin): `LOW_WATER_TOKENS` moves 350k → **200k**, preserving both invariants L < ceiling (spec §4) and L = band floor − 50k (as 350k was to the 400k floor) — leaving L at 350k would make the H1 GO bar "≥90% of candidates land at B ≤ L" vacuously true for every in-band candidate (B < A ≤ 350k), destroying the criterion. `MIN_NET_SAVING_TOKENS` (M = 40k) and `DEFAULT_HIGH_WATER` (0.70 ratio) are unchanged. Reason: organic workspace traffic rarely reaches 400–500k (the recorded H1 data-availability risk), making GO practically unreachable without a separately authorized spend campaign. The §1 empirical degradation claim (~400–500k) is unchanged; H1 establishes economics in the band real conversations reach, and any residual 400–500k product claim shifts to H3 apply-trial evidence. §7.1/§11.6. Code task: `h1-band-250-350`.
