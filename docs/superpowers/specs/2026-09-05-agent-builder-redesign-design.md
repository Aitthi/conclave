...
| D5 | **Type + CLI kind collapse into one "Runtime" segmented control** `Claude Code · Codex · Antigravity`, with a muted caption "Chat agent and Orchestrator are coming soon." The `agentType` state stays `"cli"`; the chat/orchestrator/custom "SOON" cards and the Custom tab are removed from the UI. Human ruling 2026-09-05. | Keeping SOON cards rejected: they are non-interactive and cost a full row. |
...
| D7 | **Readiness is computed per section** and shown as a dot on each rail item; the footer left slot shows the first blocking reason (`Name required`, `Model required`, `Install agy to continue`) or `Ready to create` / `Ready to save`. The primary button is disabled while any section blocks. | "2 of 4 sections ready" counter rejected: says how many, not what. |
...
| runtime | `model` non-empty AND (not Antigravity OR `cliAvailability.state === "ready"`) | Antigravity `cliAvailability.state === "missing"` |
| skills | always | never |
| position | always | never |

`firstBlocker` returns, in order: `"Name required"`, `"Install agy to
continue"`, `"Checking agy…"`, `"Model required"`; else `null`. The footer
shows the blocker in `text-text-tertiary` (danger colour for the agy-missing
case) or `Ready to create` / `Ready to save`. The primary button is disabled
when a blocker exists or `saving` is true; its label logic is unchanged.
Today the button is only disabled for saving/Antigravity and an empty name
fails inside `handleSave`; after this change the empty-name case is blocked
before the click. `handleSave`'s own name check stays as a guard.

### Runtime control (D5)

The segmented control drives `cliKind` directly (`claude-code`, `codex`,
`antigravity`). `agentType` is always `"cli"`; the `setAgentType` state and the
`showCliConfig` flag stay so the non-CLI Model/API fallback branch still
...
the existing Custom args input and the Custom environment toggle + textarea,
moved verbatim.
...
1. `agent-builder-canon` — Arta: `design/screens/agent-builder.tsx` per D10,
   pinned SHA reported in the READY note. Escalation: Detoro.
2. `agent-builder-redesign` — Dew: implementation per this spec and the pinned
   canon; boundary `src/components/Builder.tsx`, `src/components/builder/**`,
   `src/components/AppShell.tsx`, `scripts/uishot.mjs`, `CLAUDE.md`.
   Created only after task 1 is READY. Review: Mellow. Escalation: Detoro.
