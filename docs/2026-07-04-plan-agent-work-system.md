...
- **LIVE-PIXEL CONFIRMS WAIVED BY HUMAN (2026-07-04, post-r5 relaunch)** — r5 verified live (pid start + conclave.sock mtime both moved; binary 47,262,112B matches the notarized build). Arta's live capture was BLOCKED: no macOS Screen Recording permission in any agent terminal (lead reproduced the same `could not create image from window` failure), no listening TCP/CDP port on the app process, Arta viewer :7317 is proto-only. Human was offered permission-grant / manual-screenshot / waive and chose WAIVE. Final design verdicts are therefore the static fidelity ACCEPTs (LaneBoard @ review:aws/d-design vs canon fa4929b; memory-graph ACCEPT 0 findings). bb keys `review:aws/d-live` + `review:memory-graph-live` record WAIVED-BY-HUMAN. Residual (accepted) risk: live-data render differences would not have been caught. If live-pixel checks are ever wanted later, the unblock is Screen Recording permission for the agents' terminal app.
...
- **Lane C MERGED @ ef07b35** (e1ab408, Guetta; Mellow LAND PASS 0 blocking; lead-reproduced 475/21/4 + clippy clean). Two latent Lows on record, deferred follow-ups, non-blocking:
  - L1: `lane guard install` writes `<common>/hooks/pre-commit` directly and does not honor `core.hooksPath` — silent no-op if a checkout ever sets it (e.g. husky). Inert in this repo today. Follow-up: warn-if-set at install time.
    **CLOSED @ 3580606** (lane 0d3fe92, Dabin; Mellow LAND PASS 0 blocking; gates lead-reproduced 563/40/5 + clippy clean). Ruling applied: warn on stderr, still install, exit 0. Two non-blocking Lows on record (Mellow): warning also fires when core.hooksPath already points at the shared hooks dir (self-hedging text, false-positive only), and no explicit negative-assertion test for the silent unset path.
