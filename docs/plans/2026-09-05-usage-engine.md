...
## Amendment C6 — durable reconciliation evidence (migration 0032)
Review a12f77f2 C6 and ruling 8b7dc696: a bounded recent-group cache cannot prove Claude block agreement after eviction or across restart, so the evidence lives on the event. The canonical schema amendment is docs/plans/2026-09-05-usage-reconciliation-schema.md (supplemental task usage-reconciliation-schema, same implementer and worktree, owning only 0032_model_usage_reconciliation.sql and itself). This parent task owns the db.rs registration (schema 32), the two NewUsageEvent/StoredIdentity fields (stop_reason bounded to 128 chars, source_uncached_input_tokens bounded 0..2^40), the Claude parser storing both, and apply_scan comparing response id, served model, stop reason, uncached input and every stored input/output/cache counter. An oversized stop reason is a bounded diagnostic (partial coverage), never a truncated value; a legacy NULL-evidence row stays conflicting on replay; an agreeing replay changes only occurred_at (forward, canonical ms) and never clears a prior conflict. Direct in-process sources store NULL evidence: they are observed once and never replayed.
...
## Residual acceptance clarification — ruling 1db7827f
The existing C3 requirement includes every owner declaration within one structurally valid text field, as well as declarations across records. A first-match extractor is insufficient: a registered owner followed by an unregistered owner must prevent attribution. The existing C5 complete-observation proof requires a consumed record boundary on both the tick reading a partial record and all subsequent idle ticks. Retain pending bytes and oversized-record skip continuation without rereading unchanged bytes; only newly appended bytes are charged, and completing the record imports its event exactly once. Zero-byte EOF is clean only when no unconsumed continuation or damage remains.

Exact regression supplement: `/tmp/aoki-spec-residual-probes-v2.patch`, artifact `22462f87-45f6-4bcd-823a-eb101f1be655`, SHA256 `e611b5588863705ad238a7b2b2d83331455d5eba235eb8967df30aa08b93f9e6`. Apply both correct-behavior test bodies without weakening assertions. At final HEAD record `conclave task gate 11ecf99b-53f4-4c24-b538-b19e5933a9e3 usage-engine-v2 -- cargo test --manifest-path src-tauri/Cargo.toml --lib residual_probe_ -- --nocapture --test-threads=1`; exactly two tests must pass, in addition to the prior five `spec_review_`, three `review_acceptance`, receiver-loss, and full build/test gates. Root independently reproduced both RED at 7b409a12. These defects misattribute usage and falsely prove coverage, so ruling f9abdd62 keeps collectors in the lane until fixed; they cannot ship as documented under-collection.
