...
### Review correction: global definition deletion
Armin identified bypass 998211c5 at commands/agent.rs::delete: public agentDef.delete calls the internal removal bypass across all assigned workspaces. Explicitly reject the entire definition deletion if any affected workspace is archived. Validate every affected workspace after acquiring its existing stable-ordered lifecycle write locks and BEFORE any instance teardown, database deletion or keychain removal. A mixed active+archived definition must fail with Restore-first guidance and leave active runtimes and every instance/session/definition intact. Keep workspace.delete's internal cleanup for an explicitly deleted archived workspace permitted. Add a production-router regression proving the mixed case has no partial removal/termination, plus successful definition deletion after restore. This named path was missing from the original lead checklist; credit Armin, and audit callers of internal removal helpers as well as public routes in future lifecycle plans.

### Review correction: formatting scope
Armin identified unrelated formatting hunk 1060a02c at repo/workspace_agent.rs in an existing list_all_ids test. Revert that hunk. Shared repository/command diffs must contain only semantic feature changes and directly necessary local formatting. When current rustfmt would rewrite unrelated baseline lines, report the verified baseline discrepancy rather than sweeping it into this lane to obtain a green whole-file check. Whitespace/diff checks still must pass.
