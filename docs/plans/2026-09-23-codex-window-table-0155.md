...
## Amendment 2 (Detoro, 2026-09-23) — challenge 5f6cca31 UPHELD, replaces Amendment 1 item 3 and refines item 1
Evidence (Mellow, independently re-derived by Detoro from `~/.codex/sessions`): gpt-5.6-luna served turns up to 616,384 input tokens (18 turns above 372K, 0 errors, codex-cli 0.153.4) and gpt-5.6-sol up to 526,265 (42 turns above 372K, 0 errors, 0.154.0). The July ~372K server-side cap (issue #31860) is REFUTED, not merely unverified. Mellow also confirmed from codex source at rust-v0.155.1 that `with_config_overrides` sets `context_window = min(cfg, max_context_window)` and `auto_compact_token_limit = min(cfg, 0.9 × resolved)`.

3'. `gpt-5.6-sol | gpt-5.6-terra | gpt-5.6-luna` → `Some(872_000)`, same arm or same value as the GPT-6 family. Comment: 0.155.1 catalogue default 272K / max 872K; July 372K cap refuted by local sessions serving 526K–616K with no errors (dates above); one sentence of history for the old 372_000, no more.
1'. Comment wording for item 1: say "codex reports 828,400 usable (Conclave's meter shows 816,400 after the 12K baseline)" so nobody compares the two numbers.
Tests: `gpt_5_6_family_*` asserts 872_000 (rename accordingly); the Builder hint in `src/components/builder/RuntimeSection.tsx` that says "Some models are server-capped below 1M (GPT-5.6 ≈ 372K)" is now FALSE — change that hint to "Codex clamps to each model's catalogue max (GPT-6 / GPT-5.6 872K, GPT-5.5 272K)." RuntimeSection.tsx joins the boundary for that one string only; run `pnpm uishot builder-edit --viewport 1440x1900` and Read the PNG per the UI Pixel Gate, since a copy change touches `src/` UI.

## Amendment 3 (Detoro, 2026-09-23) — challenges 2abb8077 (Dew) and 030a4ed6 (Mellow) UPHELD: Amendment 2's RuntimeSection step is withdrawn
The "GPT-5.6 ≈ 372K" hint Amendment 2 quoted was already replaced in 921524f; the current hint (RuntimeSection.tsx:676, "GPT-6 / GPT-5.6 report ≈ 828K usable; the meter shows …") stays true under the new table. No UI edit, RuntimeSection.tsx is NOT in boundary, no uishot gate. The lane stays Rust-only (codex_models.rs plus the one instance.rs test). Lead error: the string was cited from the 2026-09-07 plan text instead of current source.
